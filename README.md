...
```mermaid
flowchart TD
    A["PRD / 问卷 / 用户补充材料"] --> A1["最小上下文模式<br/>minimal-drafting-playbook.md"]
    A1 --> A2["文档适格性闸门<br/>document-scope-gate.md"]
    A2 --> B["事实抽取层<br/>product-intake-schema.md"]
    ...
    K["来源约束层<br/>source-policy.md"] --> A2
    K --> B
    K --> D
    K --> I
```
...
- [SKILL.md](SKILL.md)
- [references/minimal-drafting-playbook.md](references/minimal-drafting-playbook.md)
- [references/document-scope-gate.md](references/document-scope-gate.md)
...
1. 读取 PRD 和补充材料。
2. 先过 [references/document-scope-gate.md](references/document-scope-gate.md)，判断能否诚实地产出完整协议。
3. 用 [references/product-intake-schema.md](references/product-intake-schema.md) 抽取事实。
4. 用 [references/template-selection.md](references/template-selection.md) 和 [references/online-reference-matrix.md](references/online-reference-matrix.md) 选择 archetype。
5. 用 [references/source-capture-qa.md](references/source-capture-qa.md) 判断对应参考源能不能信。
6. 需要时回看 [references/source-captures/](references/source-captures/) 里的单个 archetype 正文，而不是通读全部知识库。
7. 用 [references/agreement-writing-style.md](references/agreement-writing-style.md) 决定标题、开头块、章节命名、专业表达方式。
8. 按 [references/output-contract.md](references/output-contract.md) 输出最终两份协议。
...
- 最小上下文模式：[references/minimal-drafting-playbook.md](references/minimal-drafting-playbook.md)
- 文档适格性闸门：[references/document-scope-gate.md](references/document-scope-gate.md)
